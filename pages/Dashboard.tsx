...
import { getConcurrencyCount } from '../utils';
import { ChevronLeft, ChevronRight, AlertTriangle, Calendar, LayoutGrid, Users } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const getHolidayColorClass = (location: string) => {
    switch (location) {
      case 'BR': return 'bg-green-100 text-green-700 font-bold border-green-200'; // Nacional Brasil - Verde
      case 'MAD': return 'bg-yellow-100 text-yellow-700 font-bold border-yellow-200'; // Espanha - Amarelo
      case 'MEX': return 'bg-red-100 text-red-700 font-bold border-red-200'; // Mexico - Vermelho
      case 'SP': return 'bg-fuchsia-100 text-fuchsia-700 font-bold border-fuchsia-200'; // SP - Fuscia
      case 'BH': return 'bg-purple-100 text-purple-700 font-bold border-purple-200'; // BH - Roxo
      default: return 'bg-gray-200 text-gray-600 font-bold border-gray-300'; // Global or fallback
    }
  };
  ...
  const renderMonthView = (specialty: Specialty) => {
    ...
    // If showing ALL, hide empty sections. If showing specific tab, show it even if empty to indicate no employees.
    ...
    const dailyUsage = daysInMonth.map((day: Date) => {
      ...
      return { day, count, warning: count > 4 }; // Warning if > 4 strictly, usually rule is max 4
    });
            ...
            {employees.length === 0 ? (
              ...
              employees.map(emp => (
                <div key={emp.id} className="flex hover:bg-gray-50 h-10 items-center">
                  ...
                  {daysInMonth.map((day: Date) => {
                    // Find request for this day/employee
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-800">Visão Geral (Timeline)</h2>

      {/* --- DASHBOARD CONTROLS (View, Date, Legend) --- */}
      <div className="bg-white p-4 rounded-lg shadow-sm space-y-4">
        ...
        {/* Enhanced Legend (Always Visible) */}
